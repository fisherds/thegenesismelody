...
function toggleVerseSelection(chapter, verse, isSelected) {
    ...
    if (verseItem) {
        ...
        // Also update the checkbox input's checked state
        const checkbox = verseItem.querySelector('input[type="checkbox"]');
        if (checkbox) {
            checkbox.checked = isSelected;
        }
    }
...
async function handleFeedbackSubmit() {
    ...
    try {
        ...
        for (let i = 0; i < Math.min(5, currentResults.length); i++) {
            ...
            // This includes: id, title, text, hebrew, strongs, verses, verse_display, etc.
